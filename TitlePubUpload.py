...
my_file = Path('%s' % (xptPub))
if my_file.is_file():
    os.remove('%s' % (xptPub))
else:
    # print("The file does not exist")
    print("No Publisher Exception File found ....")

print("Creating Publisher Exception File ....")

# For first upload of Developer worksheet
df = pd.read_excel('%s' % (dir9), sheet_name='Publishers', engine='openpyxl')

df = df.replace(np.nan, ' ', regex=True)
df_Header = df.columns.ravel()

# Create Developer Exception Worksheet
workbook = xlsxwriter.Workbook('%s' % (xptPub))
...
for ir in range(0, len(df)):
    ...
    for ic in range(0, len(df.columns)):
        ...
        if ic == 1:
            ...
            Publishers = str(df.iat[ir, 4])
            if Publishers in ('0', '0.0', '', ' ', '$N/A'):
                Publishers = ''
            ...
            # A. Database is not blank, NewValue file upload
                ...
                for row in result:
                    ...
                    if str(Name) == str(db_Name) and str(Publishers) == str(db_Publisher) and str(IGDB) == str(db_IGDB):
                        itemFound = True
