'''
The objective of this script is to insert records from the Porting Developers worksheet of GameTitles_MasterList file.
...
my_file = Path('%s' % (xptPortDev))
if my_file.is_file():
    os.remove('%s' % (xptPortDev))
else:
    # print("The file does not exist")
    print("No Porting Developer Exception File found ....")

print("Creating Porting Developer Exception File ....")

# For first upload of Developer worksheet
df = pd.read_excel('%s' % (dir9), sheet_name='PortingDevelopers', engine='openpyxl')

df = df.replace(np.nan, ' ', regex=True)
df_Header = df.columns.ravel()

# Create Developer Exception Worksheet
workbook = xlsxwriter.Workbook('%s' % (xptPortDev))
...
def InsertRecord(db_ID1, TId, Name, IGDB, NewZoo, PortDev, StudioID, VTSID, ExptRow):
    cursor = conn.cursor()

    try:
        # print("First Upload..... ")
        cursor.execute(
            "INSERT INTO GamesTitles_PortDev(ID, TitleID, TitleName, IGDB_Website, NewZoo_Website, PortDev, StudioID, VTSID) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            , db_ID1, TId, Name, IGDB, NewZoo, PortDev, StudioID, VTSID
        )
        ...
        print("Insert Error on TitleName_PortDev = " + str(Name))
        err = str(err)
        ...
        worksheet.write(ExptRow, 4, PortDev)
...
for ir in range(0, len(df)):
    ...
    for ic in range(0, len(df.columns)):
        ...
        if ic == 1:
            ...
            PortDev = str(df.iat[ir, 4])
            if PortDev in ('0', '0.0', '', ' ', '$N/A'):
                PortDev = ''
            ...
            if len(result) == 0:
                ...
                for row1 in result1:
                    ...
                    if str(Name) == str(db_Name1):
                        ...
                        InsertRecord(db_ID1, TId, Name, IGDB, NewZoo, PortDev, StudioID, VTSID, ExptRow)
            ...
            # A. Database is not blank, NewValue file upload
                ...
                for row in result:
                    ...
                    if str(Name) == str(db_Name) and str(PortDev) == str(db_PortDev) and str(IGDB) == str(db_IGDB):
                        itemFound = True
                        break
